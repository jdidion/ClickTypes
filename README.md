...
In Click, type conversion can be done either in a callback or by using a callable type (such as a subclass of ParamType) as the type. In ClickTypes, type conversions are performed automatically based on type annotations for callable types. However, for more complex type conversion, there are three additional methods:
...
@clicktypes.conversion(DoubleInt)
...
A `clicktypes.composite_factory` function returns a complex type. For example, the code below is equivalent to the code above:

```python
@clicktypes.composite_factory(Foo)
...
These functions can be associated with paramters in two ways. First, using the 'conditionals' and 'validations' arguments of the command decorator. These are dicts with a parameter name or tuple of paramter names being the key and the function being the value. Second, validation functions can be associated with paramters when they are decorated with `@clicktypes.validation` and the parameter type matches the type argument of the validation decorator. Multi-paramter validations can only be associated via the first method. Since conditionals are expected to be multi-valued, there is no `@clicktypes.conditional` annotation, i.e. they must always be explicitly specified.
...
@clicktypes.validation(PositiveEven)
...
```python
# test.py
@clicktypes.command(show_defaults=True)
